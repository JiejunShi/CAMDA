...
CTmat2MR_MIELD<-function(x,w="cg"){
  ...
  if(w=="cg"){
    MIELD=sum(interrupt_frag)/(sum(methyl_frag)+sum(unmethyl_frag)+sum(interrupt_frag));
  }else if(w=="1"){
    MIELD=length(interrupt_frag)/(length(methyl_frag)+length(unmethyl_frag)+length(interrupt_frag));
  };
  return(c(MethRatio,MIELD));
};
################
...
if(UseStrand==FALSE){
  write("chr\tstart\tend\tRead_count\tCT_count\tMethRatio\tMIELD",file = Output)
}else{
  write("chr\tstart\tend\tstrand\tRead_count\tCT_count\tMethRatio\tMIELD",file = Output)
};
for(i in 1:nrow(Regions)){
  ...
  if(read_count>0){
    ...
    ratios=CTmat2MR_MIELD(x=ctmat,w=Weight);
