...
## Usage
There are two scripts in MIELD toolkit. (`./scripts/functions.py` contains the functions required by `./scripts/MIELD.py`) Example files of all the input and output can be found in `./demo/`.

	$ python ./scripts/MIELD.py
 	MIELD Toolkit
	...
	$ Rscript ./scripts/ReadCT2MIELD.r -h
	Usage: Rscript ReadCT2MIELD.r [-[-help|h]] [-[-ReadCT|i] <character>] [-[-Regions|r] <character>] [-[-UseStrand|s]] [-[-Weight|w] [<character>]] [-[-Output|o] [<character>]]
		-h|--help	useage
...
#### 1.1 Generating MIELD and Mean Methylation Ratio(MethRatio) of each CpG from BSMAP alignments
	...
	$ python ./scripts/MIELD.py MIELD ./demo/example.bam hg19.fa -o example -w example -s "/path/to/samtools/v0.1.19" -x CG
...
#### 1.2 Calculating MIELD ratio of given regions from MIELD scores of CpG
	...
	$ python ./scripts/MIELD.py BedRatio ./demo/example.bed example_CpG_MIELD.tsv -o example_MIELD.tsv

#### 1.3 Calculating MethRatio of given regions from MethRatio of CpG

	$ python ./scripts/MIELD.py BedRatio ./demo/example.bed example_CpG_MethRatio.tsv -o example_MethRatio.tsv

### 2. MIELD (***M***, ***U***, and ***I*** fragments are unweighted)
#### 2.1 Generating ReadCT file from BSMAP alignments

	$ python ./scripts/MIELD.py ReadCT ./demo/example.bam hg19.fa -o example_ReadCT.tsv -s "/path/to/samtools/v0.1.19" -x CG
...
#### 2.2 Calculating MIELD and MethRatio from ReadCT file
	...
	$ Rscript ./scripts/ReadCT2MIELD.r -i example_ReadCT.tsv -r ./demo/example.bed -w 1 -o example_MIELD_weight_1.tsv
